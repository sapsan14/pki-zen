...
Recommended primary: **`zen.tyche.institute`** on Cloudflare Pages.
...
| Option | Cost | Verdict |
|---|---|---|
| `zen.tyche.institute` (subdomain under the Tyche Institute umbrella) | **€0** (already own the domain) | ⭐ **Recommended.** Clean Cloudflare DNS, strong institutional umbrella, no base-path gymnastics. |
...
**Decision:** `zen.tyche.institute` primary, `pki-zen.pages.dev` preview.
...
5. Custom domain: CF dashboard → `pki-zen` → **Settings → Domains & Routes → Add Custom Domain → `zen.tyche.institute`**. `tyche.institute` should already be on Cloudflare DNS; if not, add the zone first or point a proxied `CNAME` at the Worker route target.
...
5. Custom domain: Workers → `pki-zen` → **Settings → Domains & Routes → Add Custom Domain → `zen.tyche.institute`**.
...
6. Add custom domain `zen.tyche.institute` under **Custom domains**.
...
- Update `README.md` `zen.tyche.institute` badge colour to green if you want a green-means-live vibe. The current palette is already book-like, so this is cosmetic.
...
git tag -a v1.0.0 -m "PKI-ZEN v1.0.0 — the complete trilingual sūtra" && \
git push origin v1.0.0
# Then: Cloudflare Pages rebuilds zen.tyche.institute automatically.
```
